    <div class="image-title-centered">
        ...
        <h1 id="event-title" class="centered">{{ old('name') ?? $event->name ?? 'Preview Title'}}</h1>
        @if (old('_token') !== null)
        <h2 id="event-date" class="bottom-right">{{ date('d/m/Y', strtotime(old('event_date_start'))) }} - {{ date('d/m/Y', strtotime(old('event_date_end'))) }}</h2>
        @else
        <h2 id="event-date" class="bottom-right">{{ date('d/m/Y', strtotime($event->event_date_start ?? date('Y-m-d\TH:i'))) ?? 'Preview Date' }} - {{ date('d/m/Y', strtotime($event->event_date_end ?? date('Y-m-d\TH:i'))) ?? 'Preview Date' }}</h2>
        @endif
    </div>
    <style nonce="{{ csp_nonce() }}">
        #event-title {
            color:{{ old('title_color') ?? $event->title_color ?? 'rgb(255,255,255)' }};
            display:{{ $event->display_title ?? 'block' }};
        }
        #event-date {
            color:{{ old('title_color') ?? $event->title_color ?? 'rgb(255,255,255)' }};
            display:{{ $event->display_title ?? 'block' }};
